...
[[ -n "$2" && "$target" == "file" ]] && { deb_dir="$(realpath "$2")"; }
[[ -n "$2" && "$target" == "dir"  ]] && { deb_file="$(realpath "$2")"; }

# Defaults
if [[ -n "$deb_dir" || -n "$deb_file" ]]; then
  [[ -n "$deb_dir" && -z "$deb_file" ]] && deb_file="$deb_dir".deb
  [[ -n "$deb_file" && -z "$deb_dir" ]] && deb_dir="${deb_file%.deb}"  # "file.deb" -> "file"
fi
...
function repack() {
  ...
  rm -rf "$dir"/control.tar.xz
  rm -rf "$dir"/data.tar.xz
}
